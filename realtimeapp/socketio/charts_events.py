...
def get_chart_data():
    ...
    if THERMOHYGRO:
        ...
        last=last_reading()
        emit('last_reading', {'label': str(last['date'].isoformat()), 'dataa':  last['temperature'], 'datab':  last['humidity'], 'datac' : last['moisture1'], 'datad' : last['moisture2'] },  room=room)
        emit('my_chart_init', {'label': transposed_readings[1].tolist(),
             ...
             'datac':  transposed_readings[4].tolist(),
             'datad':  transposed_readings[5].tolist() },
              room=room)
    ...
    # Create placeholder blanck chart when database is empty 
        ...
        emit('last_reading', {'label': [] , 'dataa': [] , 'datab': [], 'datac' : [], 'datad' : [] }, room=room)
        emit('my_chart_init', {'label': [] , 'dataa': [] , 'datab': [], 'datac' : [], 'datad' : [] }, room=room)
...
def emit_new_reading(reading):
    ...
    if  THERMOHYGRO:
        ...
        socketio.emit('last_reading', {'label': reading['date'] , 'dataa':  reading['temperature'], 'datab':  reading['humidity'], 'datac' : reading['moisture1'], 'datad' : reading['moisture2'] }, namespace="/charts", room=sensor_name)
        socketio.emit('my_chart', {'label': reading['date'] , 'dataa':  reading['temperature'], 'datab':  reading['humidity'], 'datac' : reading['moisture1'], 'datad' : reading['moisture2'] }, namespace="/charts", room=sensor_name)
